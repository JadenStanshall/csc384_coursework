...
from search import *
# import everything from search file


class SokobanState(StateSpace): # inheret from state space
    ...
    def __init__(self, action, gval, parent, width, height, robots, boxes, storage, obstacles):
        ...
        '''
        # additional attributes for sokoban problem specifically
    ...
    def successors(self): # overwritten
        '''
        Generates all the actions that can be performed from this state, and the states those actions will create.
        '''
        # generatea ll state spaces we can get to from the current state space
        # see all actions the robot can do (see over every robot, and make it move in every possible direction and check the new location)
        # check whether the new successor state is valid (wrt obstacles, box stacking)
        successors = []
        transition_cost = 1
        moved_boxes = frozenset()

        for robot in range(0, len(self.robots)): # checks to see if the new state voilates contraints of the game set up
            for direction in (UP, RIGHT, DOWN, LEFT):
                ...
                new_state = SokobanState(str(robot) + " " + direction.name, self.gval + transition_cost, self,
                                         ...
                                         self.obstacles)
                successors.append(new_state) # store new state in successors list

        return successors

    def hashable_state(self): # assign a unique has to each state to compare
        '''Return a data item that can be used as a dictionary key to UNIQUELY represent a state.'''
        return hash((self.robots, self.boxes))

    def state_string(self): # graphical representation of state
...
def sokoban_goal_state(state): # only cehcks if you reached a goal state or not (goal state -> box locations = storage locations)
...
'''
class Direction(): # used to move the robots and boxes
    '''
    ...
    def move(self, location): # gets current location of the thing to move (either robot or box) and moves based on delta (1)
        '''
