    ...
    def __lt__(self, other):
        ...
        if sNode.lt_type == _SUM_HG:
            if (self.gval + self.hval) == (other.gval + other.hval):
                ...
                return self.gval > other.gval # choose the node with higher goal value (lower heuristic value)
            else:
                return ((self.gval + self.hval) < (other.gval + other.hval)) # choose the node with lower summation
        if sNode.lt_type == _G:
            return self.gval < other.gval # choose node with least goal value from fronteir
        if sNode.lt_type == _H:
            return self.hval < other.hval # choose node with least goal value from fronteir
        if sNode.lt_type == _C:
            return self.fval_function(self) < other.fval_function(other) # choose node with least goal value from fronteir
...
class Open:
    '''Open objects hold the search frontier---the set of unexpanded
       ...
       strategy'''
       # set the type of frontier based on search strategy
    ...
    def __init__(self, search_strategy):
        ...
        elif search_strategy == _BREADTH_FIRST: # use queue, first in first out
            # use queue for OPEN (first in---earliest node not yet expanded---is first out)
            self.open = deque()
            self.insert = self.open.append
            self.extract = self.open.popleft
        elif search_strategy == _UCS: # least goal value, so priority queue, orders the fronteir based on goal value
            # use priority queue for OPEN (first out is node with lowest gval)
            self.open = [] # sorted by G values
...
class SearchEngine: # use to init search, define search strat and also cycle check level (path or full graph)
    ...
    def set_strategy(self, s, cc='default'): # makes sure search strategy is one of the permissible algorithms
    ...
    def init_search(self, initState, goal_fn, heur_fn=_zero_hfn, fval_function=_fval_function):
        # init state: first state for the problem
        # goal function: bool, true if final state is goal state
        # heur functio: default zero
        # fval function: how is f computed with g and h
        """
        ...
        # END
        self.open = Open(self.strategy) # init frontier

        node = sNode(initState, heur_fn(initState), fval_function) # init first node with heur and f val
